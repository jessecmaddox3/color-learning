...
import { parseCloudConfig } from '../public/lib/cloud-config.js'
import { createCloudConnection, flushOne } from '../public/lib/cloud-transport.js'
import { openProgressStore } from '../public/lib/progress-store.js'
...
function endpoint() {
  ...
  api.fetch = async (input, init) => {
    ...
    api.row = { owner_id: api.owner, profile_id: P, game_id: 'colors', curriculum_id: 'test-v1', ...body, revision: (api.row?.revision ?? 0) + 1 }
...
async function connect(api = endpoint(), options = {}) {
  const connection = createCloudConnection(config, { gameId: 'colors', curriculumId: 'test-v1', normalize, loadSDK: () => import('@supabase/supabase-js'), nativeFetch: api.fetch, ...options })
...
test('disconnect during lazy import never constructs a client', async () => {
  ...
  const connection = createCloudConnection(config, { gameId: 'colors', curriculumId: 'test-v1', normalize, loadSDK: () => gate.promise })
  connection.disconnect()
...
test('the authenticated owner and full binding constrain all reads', async () => {
  ...
  assert.equal(request.searchParams.get('game_id'), 'eq.colors')
...
test('disconnect during REST cannot acknowledge the durable queue, even if the server committed', async () => {
  const { connection, api } = await connect()
  await login(connection)
  const store = await openProgressStore({ indexedDB: new IDBFactory(), gameId: 'colors', curriculumId: 'test-v1', normalize })
