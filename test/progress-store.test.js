...
async function setup() {
  ...
  const options = { indexedDB, dbName: 'test', gameId: 'colors', curriculumId: 'test-v1', normalize: (v) => {
...
test('missing remote snapshots cannot be normalized into a new learner', async () => {
  const store = await openProgressStore({ indexedDB: new IDBFactory(), gameId: 'colors', curriculumId: 'test-v1', normalize: (value) => value ?? snapshot() })
