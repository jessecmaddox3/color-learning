...
export function createCloudPanel(root, options) {
  const { store, durable, normalize, curriculumId, getProfile, isBusy, onChange, onRestore, onProfilesCleared, onMessage, describe } = options
  ...
  function initial() {
    ...
    const help = document.createElement('a'); help.href = 'https://github.com/jessecmaddox3/color-learning/blob/main/docs/cloud-setup.md'; help.textContent = 'Cloud setup instructions for the host'; content.append(help)
  }
  function renderLogin() {
    ...
    form.addEventListener('submit', async (event) => {
      ...
      const original = createCloudConnection(config, { gameId: 'colors', curriculumId, normalize, onStatus: (value) => { if (value === 'auth-required') say('Sign in again to finish saving.') } })
      connection = original
      ...
      if (local.conflict && original.owns(local.binding)) {
        ...
        for (const choice of remote ? ['device', 'cloud'] : ['device']) {
          button(choice === 'device' ? 'Keep this device’s progress' : 'Use the cloud progress shown above', async () => {
            ...
            if (!alive(original, token)) return
            await onChange(); await renderConnected()
